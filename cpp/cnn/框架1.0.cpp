...
struct Mat {
 public:
  ...
  float& operator[](size_t index) { return tensor[index]; }

  const float& operator[](size_t index) const { return tensor[index]; }
};
        ...
        for (int h = 0; h < mats[i].height; ++h) {
          for (int w = 0; w < mats[i].width; ++w) {
            int index = d * mats[i].channel * mats[i].height * mats[i].width +
                        c * mats[i].height * mats[i].width + h * mats[i].width +
                        w;
    ...
    for (int c = 0; c < mat.channel; ++c) {
      for (int h = 0; h < mat.height; ++h) {
        for (int w = 0; w < mat.width; ++w) {
          int index = d * mat.channel * mat.height * mat.width +
                      c * mat.height * mat.width + h * mat.width + w;
...
void preread() {
  std::string conv1_weight_path =
      "D:\\code\\VS2022\\Dogs vs Cats" PATH_SEPARATOR "layer1.0.weight.bin";
  std::string conv1_bias_path =
      "D:\\code\\VS2022\\Dogs vs Cats" PATH_SEPARATOR "layer1.0.bias.bin";
  std::string conv2_weight_path =
      "D:\\code\\VS2022\\Dogs vs Cats" PATH_SEPARATOR "layer2.0.weight.bin";
  std::string conv2_bias_path =
      "D:\\code\\VS2022\\Dogs vs Cats" PATH_SEPARATOR "layer2.0.bias.bin";
  std::string conv3_weight_path =
      "D:\\code\\VS2022\\Dogs vs Cats" PATH_SEPARATOR "layer3.0.weight.bin";
  std::string conv3_bias_path =
      "D:\\code\\VS2022\\Dogs vs Cats" PATH_SEPARATOR "layer3.0.bias.bin";
  std::string linear1_weight_path =
      "D:\\code\\VS2022\\Dogs vs Cats" PATH_SEPARATOR "fc1.weight.bin";
  std::string linear1_bias_path =
      "D:\\code\\VS2022\\Dogs vs Cats" PATH_SEPARATOR "fc1.bias.bin";
  std::string linear2_weight_path =
      "D:\\code\\VS2022\\Dogs vs Cats" PATH_SEPARATOR "fc2.weight.bin";
  std::string linear2_bias_path =
      "D:\\code\\VS2022\\Dogs vs Cats" PATH_SEPARATOR "fc2.bias.bin";
...
double conv2d(const Mat input, Mat& output, std::vector<float> weight,
    ...
    for (int d = 0; d < dim; ++d) {
      for (int c = 0; c < channel; ++c) {
        for (int h = 0; h < height; h += conv_stride[0]) {
          ...
          //  continue;
          if (h + conv_kernel_size[0] > height) continue;
          for (int w = 0; w < width; w += conv_stride[1]) {
            // if ((w + stride[1]) > width)
            // continue;
            if (w + conv_kernel_size[1] > width) continue;
            int index = d * channel * height * width + c * height * width +
                        h * width + w;
            // std::cout << index << std::endl;
            sum = 0;
            for (int i = 0; i < conv_kernel_max; ++i) {
              // std::cout << i<<' '<<dx[i] << ' ' << index + dx[i] <<
              // std::endl; std::cout << weight_pos + i << ' ' << index + dx[i]
              // << std::endl;
      ...
      for (int h = 0; h < input_h; h += mp_stride[1])  // 第一个
      {
        ...
        for (int w = 0; w < input_w; w += mp_stride[0]) {
          ...
          for (int i = 0; i < mp_kernel_max; ++i) {
            if (input[index + dx[i]] > max) max = input[index + dx[i]];
...
double linear(const Mat& input, Mat& output, std::vector<float> weight,
              std::vector<float> bias) {
  double start = get_current_time();
...
int forward(Mat& input, int i) {
  ...
  printf(
      "conv1: %.3lf, relu1: %.3lf, mp1: %.3lf\nconv2: %.3lf, relu2: %.3lf, "
      "mp2: %.3lf\nconv3: %.3lf, relu3: %.3lf, mp3: %.3lf\nflatten1: %.3lf, "
      "linear1: %.3lf, relu4: %.3lf, linear2: %.3lf\n",
      conv1_time[i], relu1_time[i], mp1_time[i], conv2_time[i], relu2_time[i],
      mp2_time[i], conv3_time[i], relu3_time[i], mp3_time[i], flatten1_time[i],
      linear1_time[i], relu4_time[i], linear2_time[i]);
  // std::cout << linear2_output[0] <<
  // std::endl;//sigmoid之后会比较接近，所以可以先看sigmoid之前的数据
  printf("all time:%.3lf\n", all_time[i]);
...
int main() {
  ...
  printf(
      "Average conv1 time: %.3lf, Average relu1 time: %.3lf, Average mp1 time: "
      "%.3lf\n",
      avgConv1Time, avgRelu1Time, avgMp1Time);
  printf(
      "Average conv2 time: %.3lf, Average relu2 time: %.3lf, Average mp2 time: "
      "%.3lf\n",
      avgConv2Time, avgRelu2Time, avgMp2Time);
  printf(
      "Average conv3 time: %.3lf, Average relu3 time: %.3lf, Average mp3 time: "
      "%.3lf\n",
