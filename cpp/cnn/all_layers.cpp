...
int conv2d(const Mat input, Mat& output, std::vector<float> weight,
...
int linear(const Mat& input, Mat& output, std::vector<float> weight,
           std::vector<float> bias) {
  ...
  //  printf("%-25s,in_channels:%-4d, out_channels:%-4d, input_h:%-4d
  //  ,input_w:%-4d ,out_h:%-4d ,out_w:%-4d ,time=%fms\\n",  input.c, output.c,
  //  input.h, input.w, output.h, output.w, end - start);
  return 0;
}
